<template>
  ...
  <v-container>
    <v-container class="d-flex pb-6 pt-16 mt-6">
      <h1 class="text-medium-emphasis"><span class="text-green">U</span>sers</h1> 

      <v-spacer></v-spacer>

      <v-btn to="/Admin/Addstudent" variant="tonal" class="ma-2 z-index" color="green">
        Add User<v-icon icon="mdi-plus" end></v-icon>
      </v-btn>
    </v-container>
    ...
    >
      ...
      <tbody>
        <tr v-for="Student in serverItems" :key="Student.id">
          <td>{{ Student.id }}</td>
          <td>{{ Student.name }}</td>
          <td>{{ Student.email }}</td>
          <td>{{ Student.phone }}</td>
    ...
    async function fetchPageData() {
      ...
      try {
        const { data, error, count } = await supabase
          .from('Students')
          .select('id, name, email, phone, address, course', { count: 'exact' })
